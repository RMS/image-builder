...
def host():
    docker_id = subprocess.check_output(
        'sudo docker run -d --name test -e GIT_CRYPT_KEY={} eastus-artifactory.azure.rmsonecloud.net:6001/buildbox:{} sleep 90'.format(os.environ['GIT_CRYPT_KEY'], os.environ['CIRCLE_BRANCH']).split()).decode().strip()
