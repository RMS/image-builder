...
def host():
    ...
    docker_id = subprocess.check_output(
        'docker run -d --name test -e GIT_CRYPT_KEY={} eastus-artifactory.azure.rmsonecloud.net:6001/buildbox:{} sleep 5000'.format(os.environ['GIT_CRYPT_KEY'], os.environ['TAG']).split()).decode().strip()
